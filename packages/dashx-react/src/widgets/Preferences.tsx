...
import useDashXProvider from '../hooks/use-dashx-provider.js';
...
interface PreferencesProps {
  renderPreferences: ({
    ...
    onChange: (e: ChangeEvent<HTMLInputElement>, p: [string, StoredPreference]) => void;
    loading: boolean;
    error: any;
  }) => JSX.Element;
}
    ...
    return (
      <>
        {preferences.map((preference) => (
          <Preference key={preference[0]} preference={preference} onChange={onChange} />
        ))}
      </>
    );
...
function Preferences({ renderPreferences = defaultRenderPreferences }: PreferencesProps) {
  ...
  return renderPreferences({
    preferences:
      preferences && Object.entries(preferences).map(([key, value]: any) => [key, value]),
